 ...
 */
function cassert(condition: any, message?: string): asserts condition {
  if (!condition) {
 ...
 */
function encodeBase64(buffer: Uint8Array, length: number): string {
  let off = 0;
  let rs = '';
...
class BlowfishEngine implements Disposable {
   ...
   */
  public enhancedKeySchedule(salt: Uint8Array, password: Uint8Array): void {
    const offP = { offset: 0 };
   ...
   */
  public key(data: Uint8Array): void {
    const offP = { offset: 0 };
    for (let i = 0; i < this.P.length; i++) {
 ...
 */
function genSalt(cost: number = 10): string {
